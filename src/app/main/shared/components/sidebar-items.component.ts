...
@Component({
  ...
  template: `
    ...
    <div class="flex flex-row items-center" *ngFor="let item of items">
      ...
      [routerLink]="item.route" class="material-icons relative rounded-full h-14 w-14 flex items-center justify-center p4 hover:bg-slate-300 hover:bg-opacity-10 cursor-pointer">
        {{item.icon}}
      </a>
